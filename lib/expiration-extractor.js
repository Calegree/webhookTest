...
async function extractExpirationDate(buffer) {
  ...
  // Estrategia 2: Convertir PDF a imagen con sharp (requiere poppler)
  if (isPdf) {
    try {
  ...
  // Estrategia 3: Extraer imágenes embebidas del PDF y OCR sobre ellas
  if (isPdf) {
    try {
  ...
  // Estrategia 4: Buffer es imagen directamente (no PDF)
  if (!isPdf) {
    try {
