...
function findExpirationInLines(lines) {
  ...
  if (allDates.length > 0) {
    ...
    console.log(`   📅 Fecha más futura (fallback): ${fecha} (de ${allDates.length} fechas detectadas)`);
    return fecha;
  }
...
async function extractExpirationDate(buffer) {
  ...
  if (!isPdf) {
    try {
      // Preprocess: upscale 2x + grayscale + normalize + sharpen.
      // Upscale ayuda a Tesseract cuando el texto es pequeño (fechas en carnet).
      ...
      if (text.length > 10) {
        console.log(`   🔤 Texto OCR (primeros 400 chars): ${text.substring(0, 400).replace(/\n/g, " | ")}`);
      }
      const lines = text.split("\n").map(l => l.trim()).filter(Boolean);
      const fecha = findExpirationInLines(lines);
      if (fecha) {
        console.log(`   📅 Fecha extraída via OCR directo (preprocessed)`);
        return fecha;
      }
    } catch (err) {
