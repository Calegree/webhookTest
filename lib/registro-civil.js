...
async function verificarCertificado(folio, codigoVerificacion) {
  ...
  try {
    ...
    if (
      ...
      // Estrategia 2: Buscar iframe/embed/object con src del PDF
      if (!pdfBuffer) {
        try {
          ...
          if (pdfUrl && pdfUrl.startsWith("http")) {
            ...
            if (pdfRes) {
              ...
              if (body && body.length > 500) {
                pdfBuffer = body;
                console.log(`   📄 PDF descargado desde iframe: ${pdfBuffer.length} bytes`);
              }
            }
            await pdfPage.close();
          }
        } catch (err) {
          console.warn(`   ⚠️ Estrategia iframe falló: ${err.message}`);
        }
      }
