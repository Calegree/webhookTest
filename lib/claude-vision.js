...
// ─── OCR para HVC en formato imagen (JPEG/PNG) ─────────────────────────────
...
async function ocrImageBuffer(buffer) {
  try {
    ...
    const preprocessed = await sharp(buffer)
      ...
      .sharpen()
      .toBuffer();
    const result = await Tesseract.recognize(preprocessed, "spa", { logger: () => {} });
    return result.data.text || "";
  } catch (err) {
    console.warn(`   ⚠️ OCR del HVC falló: ${err.message}`);
    return "";
  }
}

// ─── Fallback: Groq (gratis) ───────────────────────────────────────────────

/**
 * Llama a Groq con texto (ya extraído por PDF o por OCR) para que devuelva
 * los datos estructurados del HVC en formato JSON.
 */
async function extractWithGroqFromText(text) {
...
/**
 * Extrae datos de la HVC. Acepta PDF o imagen (JPEG/PNG).
 * - Si es PDF: usa extracción directa de texto (gratis, rápido).
 * - Si es imagen: hace OCR sobre la imagen para obtener texto.
 * - En ambos casos: aplica regex sobre el texto; si falta folio/CVE, pasa
 *   el texto a Groq como fallback.
 */
async function extractLicenseData(buffer) {
  ...
  if (isPdf) {
    ...
    console.log("📝 HVC en formato imagen, ejecutando OCR...");
    text = await ocrImageBuffer(buffer);
    console.log(`   🔤 OCR extrajo ${text.length} caracteres del HVC`);
  } else {
    console.warn(`⚠️ Formato de HVC desconocido (header ${header}), intentando como PDF igual`);
    try {
      text = await extractTextFromPdf(buffer);
    } catch {}
  }
  ...
  if (data.folio && data.codigo_verificacion) {
    console.log(`✅ Extracción por ${isPdf ? "texto PDF" : "OCR"} exitosa: Folio=${data.folio}, CVE=${data.codigo_verificacion}`);
    return data;
  }

  // Fallback a Groq si falta folio o CVE y tenemos algo de texto
  if (!text || text.trim().length < 30) {
    console.log("⚠️ Sin texto suficiente para pasar a Groq, retornando datos parciales");
    return data;
  }

  console.log("⚠️ Extracción por regex incompleta, intentando con Groq...");
  ...
  try {
    const groqData = await extractWithGroqFromText(text);
    // Merge: preferir datos del regex, completar con Groq
