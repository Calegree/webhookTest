...
// ─── UTA ─────────────────────────────────────────────────────────────────────
async function validarUTA(codigo) {
  return withBrowser(async (page) => {
    console.log("   🌐 Navegando a zapahuira.uta.cl/vd...");
    await page.goto("https://zapahuira.uta.cl/vd", { waitUntil: "networkidle", timeout: 30000 });
    await page.waitForTimeout(1000 + Math.random() * 1000);
...
async function validarUCV(numero, codigo) {
  return withBrowser(async (page) => {
    console.log("   🌐 Navegando a certificados.ucv.cl...");
    await page.goto("https://certificados.ucv.cl", { waitUntil: "networkidle", timeout: 30000 });
    await page.waitForTimeout(1000 + Math.random() * 1000);
    ...
    const bodyText = await page.textContent("body");
    return analizarResultado(bodyText, "UCV");
  });
}
...
async function validarTitulo(universidadKey, datos) {
  switch (universidadKey) {
    ...
    case "uta":
      return validarUTA(datos.codigo);
    case "ucv":
